...
export const AIBriefing: React.FC<AIBriefingProps> = ({ briefing, themeColor, isLightMode, onRefresh }) => {
  return (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-lg pointer-events-none flex justify-center">
      <div 
        onClick={onRefresh}
        className={`px-4 py-2 rounded-md border backdrop-blur-md cursor-pointer pointer-events-auto group transition-all duration-500 flex items-center space-x-3 overflow-hidden ${isLightMode ? 'bg-white/80 border-zinc-200' : 'bg-black/60 border-neutral-800 hover:border-neutral-600'}`}
        ...
        <div className="flex-1 overflow-hidden">
          <AnimatePresence mode="wait">
            <motion.p 
              ...
              exit={{ y: -10, opacity: 0 }}
              className={`text-[9px] sm:text-[10px] uppercase tracking-[0.2em] font-bold leading-tight truncate ${isLightMode ? 'text-zinc-600' : 'text-zinc-300'}`}
            >
